...
1. `BoardLayer`'s only prop is the active theme's `boardStyle`, so `GameState`
   ...
   on `.stage__layer--board` from before Start until the first apple is eaten
...
2. The engine keeps every surviving segment's `Point` object identity across a
   ...
   element from before Start until the first apple is eaten and compares it by
   identity at that point, so a rebuilt head node instead of a coordinate
   rewrite fails the gate. The `<For>`
